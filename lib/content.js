...
const getEntryExcerpt = async (entry, contentType) => {
  const markdown = await fs.readFile(path.join(dataDir, contentType, entry.file), 'utf8')
  return renderExcerpt(markdown)
}

export async function getAllMeta(contentType) {
  ...
  const allEntries = await mapPromise(async entry => {
    ...
    const withSeries = assoc('series', parsedMeta?.series?.[entry?.series] ?? null, withImages)
    const withExcerpt = assoc('excerpt', await getEntryExcerpt(withSeries, contentType), withSeries)
    return withExcerpt
  }, content)
  return allEntries
}
...
export async function getContentBySlug(contentType, slug) {
  try {
    ...
    return { ...entry, content: await renderMarkdown(entryData), date, published: entry?.date, cover, placeholderImage }
  } catch (error) {
